...
This variable controls minimal thickness of lines that are used for shading. Below this value lines are not getting thinner, but become dashed instead, maintaining roughly the same amount of ink per unit length as a thinner line would take. Default value is one third of a point:
...
\subsection{pulleyWheel (\emph{radius})}
This macro returns a \texttt{picture} of a pulley wheel with specified \texttt{radius}.
